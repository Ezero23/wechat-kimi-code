...
async function runSetup(): Promise<void> {
  ...
  const workingDir = await promptUser('请输入工作目录', homedir());
...
async function runDaemon(): Promise<void> {
  ...
  async function drainQueue(): Promise<void> {
    ...
    while (messageQueue.length > 0) {
      ...
      await handleMessage(msg, account!, session, sessionStore, sender, config, routingConfig, sharedCtx, activeControllers, messageQueue);
    }
...
async function handleMessage(
  ...
  routingConfig: ReturnType<typeof loadRoutingConfig>,
  ...
  if (userText.startsWith('/')) {
    ...
    if (result.handled && result.claudePrompt) {
      await sendToClaude(
        ...
        account, session, sessionStore, sender, config, routingConfig, activeControllers,
      );
      return;
    }
...
async function sendToClaude(
  ...
  try {
    ...
    // ── Interstitial batching ──────────────────────────────────────────────
    // iLink allows ~11 messages per context_token. To avoid exhausting the
    // quota on tool_use progress ("正在调用 Bash" etc.), we buffer interstitial
    // text and flush as ONE message periodically or when a final arrives.
    const INTERSTITIAL_FLUSH_MS = 12_000;   // flush buffer every 12s
    const INTERSTITIAL_MAX_CHARS = 800;     // or when buffer exceeds this
    let interstitialBuf = '';

    function flushInterstitialBuf(): void {
      if (interstitialTimer) { clearTimeout(interstitialTimer); interstitialTimer = undefined; }
      if (!interstitialBuf.trim()) return;
      const text = interstitialBuf.trim();
      interstitialBuf = '';
      scheduleSend(text, 'interstitial');
    }

    function scheduleInterstitialFlush(): void {
      if (interstitialTimer) return; // already scheduled
      interstitialTimer = setTimeout(() => {
        interstitialTimer = undefined;
        flushInterstitialBuf();
      }, INTERSTITIAL_FLUSH_MS);
    }
    ...
    function emitText(text: string, role: 'interstitial' | 'final'): void {
      ...
      if (role === 'interstitial') {
        // Accumulate into buffer instead of sending immediately
        interstitialBuf += (interstitialBuf ? '\n' : '') + text.trim();
        if (interstitialBuf.length >= INTERSTITIAL_MAX_CHARS) {
          flushInterstitialBuf();
        } else {
          scheduleInterstitialFlush();
        }
      } else {
        // Final answer: flush any buffered interstitials first, then send final
        flushInterstitialBuf();
    ...
    // Safety net: send keepalive if nothing was sent for a while
    const SILENCE_WARNING_MS = 45 * 1000; // 45s 无输出先给用户一个进度反馈
    ...
    const decision = decideModel(routingConfig, session, prompt, !!images?.length);
    if (decision.rule !== 'manual' && decision.rule !== 'sticky') {
      logger.info('Auto-routed', { rule: decision.rule, promptLength: prompt.length, hasImages: !!images?.length, model: decision.model, routeIntent: decision.routeIntent });
    }
    ...
    // If resume failed due to session corruption (NOT quota/auth), retry without resume.
    // Quota errors (403/429) should preserve sessionId so errorFallback can resume with context.
    const isQuotaAuthError = (err: string | undefined) =>
      !!err && /403|429|usage limit|rate limit|quota/i.test(err);

    if (result.error && queryOptions.resume && !isQuotaAuthError(result.error)) {
      logger.warn('Resume failed (session corrupted), retrying without resume', { error: result.error, sessionId: queryOptions.resume });
    ...
    // If quota/auth error and errorFallback configured, switch model and retry once.
    // Keep resume intact so the fallback model inherits the conversation context.
    // Skip if the current model already goes through 9Router — it has its own
    // internal fallback chain and retrying at this level would double-hit it.
    if (result.error && routingConfig.errorFallback && !queryOptions.model?.startsWith('9router/')) {
      if (isQuotaAuthError(result.error)) {
        logger.warn('Quota/auth error, falling back to errorFallback (preserving session)', { error: result.error, fallback: routingConfig.errorFallback, resume: queryOptions.resume });
        await sender.sendText(fromUserId, contextToken, `主模型不可用，已切换到备选模型（${routingConfig.errorFallback}）处理中...`).catch(() => {});
        queryOptions.model = routingConfig.errorFallback;
        const retryResult = await claudeQuery(queryOptions);
        Object.assign(result, retryResult);
      }
    }

    // Stop periodic flush, flush remaining interstitial buffer, drain router, wait for sends
    ...
    if (pendingRetry) {
      ...
      await sender
        .sendText(fromUserId, contextToken, '⏳ 部分内容因微信单次推送上限暂存，正在尝试自动补发...')
        .catch(() => {});
      pendingRetry = null;

      // Proactive flush: after circuit breaker closes (~30s), attempt one delivery.
      // If it fails (quota truly per-token), content stays queued for next user message.
      const flushAccountId = account.accountId;
      const flushToUserId = fromUserId;
      const flushContextToken = contextToken;
      setTimeout(() => {
        flushPending(flushAccountId, flushToUserId, flushContextToken, sender)
          .then(() => logger.info('Proactive pending-queue flush completed', { accountId: flushAccountId }))
          .catch((err) => logger.debug('Proactive flush failed, will retry on next user message', {
            error: err instanceof Error ? err.message : String(err),
          }));
      }, 35_000);
    }
