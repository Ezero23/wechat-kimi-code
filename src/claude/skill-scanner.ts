...
/**
 * Scan all known skill directories for installed skills.
 *
 * Locations scanned:
 * 1. ~/.agents/skills/ (shared skills, used by kimi)
 * 2. ~/.claude/skills/ (each subdirectory)
 * 3. ~/.claude/plugins/cache/{plugin}/skills/ (each subdirectory)
 * 4. ~/.claude/plugins/cache/{plugin}/superpowers/skills/ (each subdirectory)
 */
export function scanAllSkills(): SkillInfo[] {
  ...
  for (const skill of scanDirectory(agentsSkillsDir, 1)) {
