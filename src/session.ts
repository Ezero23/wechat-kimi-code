...
export interface Session {
  ...
  model?: string;
  /** 自动路由选出的模型（一旦升级为强模型，本会话内保持，/clear 后重置） */
  autoModel?: string;
  /** 连续弱消息计数；达到阈值时解锁 sticky 强模型降级回 fast */
  autoModelWeakCount?: number;
  /** 上一轮路由选择的模型（cache-aware: 同模型复用可命中 prompt cache） */
  lastRoutedModel?: string;
  /** 最近 N 条消息的复杂度分数（用于检测对话升级趋势） */
