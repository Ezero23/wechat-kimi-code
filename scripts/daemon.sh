...
DATA_DIR="${HOME}/.wechat-kimi-code"
PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SERVICE_NAME="wechat-kimi-code"
...
macos_plist_label() {
  echo "com.wechat-kimi-code.bridge"
}
...
<dict>
  ...
  <dict>
    <key>PATH</key>
    <string>${HOME}/.kimi-code/bin:${HOME}/.local/bin:${node_bin%/*}:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin</string>
...
PLIST
  ...
  echo "Started wechat-kimi-code daemon (macOS launchd)"
}
...
macos_stop() {
  ...
  rm -f "$plist_path"
  echo "Stopped wechat-kimi-code daemon (macOS launchd)"
}
...
linux_direct_start() {
  ...
  echo "Starting wechat-kimi-code daemon (direct mode)..."
...
linux_start() {
  if linux_systemd_available; then
    ...
    echo "Started wechat-kimi-code daemon (Linux systemd)"
  else
...
linux_stop() {
  if linux_systemd_available && systemctl --user cat "${SERVICE_NAME}" &>/dev/null; then
    ...
    echo "Stopped wechat-kimi-code daemon (Linux systemd)"
  else
    linux_direct_stop
  fi
}
