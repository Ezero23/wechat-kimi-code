...
> 企业 AI 支出中，约 **60% 花在了不需要顶级模型的任务上**。
> — Cursor Router 团队基于百万级生产请求的观测（2026.07）
...
- **Cache-Aware 路由** — 边界信号时保持上一轮的模型不变，最大化 prompt cache 命中率（Anthropic/OpenAI 缓存命中可省 90% input token 费用）
- **动态升级** — 追踪对话复杂度趋势，连续 3 轮消息复杂度递增且超过阈值时自动升级到强模型
- **Sticky + 自动降级** — 升级后锁定强模型，连续 3 条闲聊自动降回快速模型，不浪费钱

配置示例（`~/.wechat-claude-code/routing.json`）：
